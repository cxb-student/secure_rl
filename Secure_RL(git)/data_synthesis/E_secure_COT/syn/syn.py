...
Format:
The format could be<secure_COT></secure_COT><secure_result>...(chose from unsafe and safe)
</secure_result>
Input Information:
[Database Schema]:
{schema}
    ...
    for idx, item in tqdm(enumerate(data), total=len(data), desc="Processing"):
        ...
        for sql_idx, sql in enumerate(sql_list):
            ...
            if sql_idx ==len(sql_list):
                ...
                continue
            if item["cot_content"] == "None" or item["cot_content"] == None:
                A_COT = ""
            else:
