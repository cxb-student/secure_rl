...
def generate_prompt(safe_condition, schema_str, exs):
    ...
    prompt_template = get_prompt_template("7", is_number, is_cells)
    if prompt_template == "None":
        return 0
    example_key = "Violence"
...
if __name__ == "__main__":
    ...
    for i in range(len(safe_conditions)):
        ...
        for _ in range(args.prompts_per_type):
            prompt = generate_prompt(safe_condition, schema_str, exs)
            if prompt == 0:
                continue
            if prompt:
                all_prompts.append(prompt)
                all_labels.append("Violence")
    ...
    #
    #     # 保存数据库ID到同一个文件中
    #     db_ids_path = os.path.join(os.path.dirname(args.output_path), "result_db_ids.json")
    #     with open(db_ids_path, "w", encoding="utf-8") as fw:
    #         fw.write(json.dumps(result_ids, indent=2, ensure_ascii=False))
    #     print(f"Database IDs saved in {db_ids_path}")
