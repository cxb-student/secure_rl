...
import os
import sqlite3
core_str = "sft_all_merged_train_text2sql"
text2sql_data_dir = f'C:\\Users\\Lenovo\\torch\\research\\NL2SQL\\Code-S\\data\\{core_str}.json'
...
def get_random_samples(table_name, cursor, sample_count=3):
    """获取表中随机的几行样本数据"""
    try:
        # 获取表中的列名
        columns_info = execute_sql(cursor, "PRAGMA table_info('{}')".format(table_name))
        column_names = [info[1] for info in columns_info]
        
        # 获取表中的总行数
        count_result = execute_sql(cursor, "SELECT COUNT(*) FROM `{}`".format(table_name))
        ...
        if total_rows <= sample_count:
            sample_sql = "SELECT * FROM `{}` LIMIT {}".format(table_name, total_rows)
        else:
            # 使用随机排序获取随机样本
            sample_sql = "SELECT * FROM `{}` ORDER BY RANDOM() LIMIT {}".format(table_name, sample_count)
        
        sample_results = execute_sql(cursor, sample_sql)
        
        # 将结果转换为列表的列表，每个内部列表代表一行的所有列值
        samples = []
        for row in sample_results:
            ...
            for i, col_name in enumerate(column_names):
                ...
                if len(value) <= 25:  # 限制值的长度
                    row_data[col_name] = value
                else:
                    row_data[col_name] = value[:22] + "..."
...
def get_db_schema_sequence(schema, db_path=None):
    ...
    for table in schema["schema_items"]:
        ...
        if db_path:
            try:
                ...
                sample_data = get_random_samples(table["table_name"], cursor)
            except Exception as e:
                print(f"无法获取表 {table_name} 的样本数据: {e}")
        ...
        for column_name, column_type, column_comment, column_content, pk_indicator in \
            ...
            if len(column_content) != 0:
                additional_column_info.append("example values : " + ", ".join(column_content))
            ...
            if sample_data:
                ...
                for i, row in enumerate(sample_data):
                    if column_name in row and row[column_name]:
                        sample_values.append(f"sample {i+1}: {row[column_name]}")
                if sample_values:
                    additional_column_info.append("samples : " + " | ".join(sample_values))
...
# 假设每个数据项都有一个db_id和对应的数据库路径
# 这里需要根据实际情况修改数据库路径的获取方式
db_root_path = 'C:\\Users\\Lenovo\\torch\\research\\NL2SQL\\Code-S\\data\\database'
...
for data in dataset:
    ...
    if 'db_id' in data:
        db_id = data['db_id']
        potential_db_path = os.path.join(db_root_path, db_id, db_id + '.sqlite')
        if os.path.exists(potential_db_path):
            db_path = potential_db_path
    
    # 生成schema序列，传入数据库路径以获取随机样本
    data["schema_sequence"] = get_db_schema_sequence(data["schema"], db_path)
...
with open(f'data_synthesis/original_data/{core_str}_with_column.json', 'w', encoding='utf-8') as f:
